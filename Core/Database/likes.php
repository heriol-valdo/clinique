...
class likes extends  Table
{
    ...
    public static function save( $council_id,$userid,$like_status,$id=null){
        $sql = 'INSERT INTO ';
        $baseSql = self::getTable().'SET council_id= : council_id,userid= :checkout_order_i ,like_status= : like_status,';
        $baseParam = ['council_id'=> $council_id,'userid'=> $userid,'like_status'=> $like_status];
        if(isset($id)){
            $sql = 'UPDATE ';
            $baseSql .= ' WHERE id = :id';
            $baseParam [':id'] = $id;
