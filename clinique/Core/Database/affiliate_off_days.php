...
class affiliate_off_days extends  Table
{
    protected static $table = 'affiliate_off_days';

    public static function save( $affiliate_id,$id=null){
        $sql = 'INSERT INTO ';
        $baseSql = self::getTable().'SET affiliate_id= :affiliate_id';
        $baseParam = ['affiliate_id'=>$affiliate_id];
        if(isset($id)){
            $sql = 'UPDATE ';
            $baseSql .= ' WHERE id = :id';
            $baseParam [':id'] = $id;
